...
from ringmoe_framework.models.core.relative_pos_bias import RelativePositionBias
from ringmoe_framework.models.layers.layers import LayerNorm, Linear, Dropout, Identity
...
class Vit(nn.Cell):
    ...
    def __init__(self,
                 ...
                 use_shared_rel_pos_bias=False, use_mean_pooling=False, predictor_layer=False,logger= None):
        ...
        self.patch_embed.set_comm_fusion(parallel_config.gradient_aggregation_group)
        ...
        self.encoder = VisionTransformer(logger=logger,**self.encoder_config)
        ...
        self.norm.set_comm_fusion(parallel_config.gradient_aggregation_group)
        self.norm.shard(((dp, 1, 1),))
        if use_mean_pooling:
            self.fc_norm = LayerNorm((embed_dim,), eps=1e-6).shard(((dp, 1),))
            self.fc_norm.set_comm_fusion(parallel_config.gradient_aggregation_group)
            self.fc_norm.shard(((dp, 1),))
            del self.norm
        else:
    ...
    def fix_init_weight(self):
        ...
        def rescale(param, layer_id):
            param.set_data(param.data / (math.sqrt(2.0 * layer_id)))
