...
class Moe_modal(nn.Cell):
    def __init__(self,
        ...
        self.special_ffn = nn.CellList()
        self.special_router = nn.CellList()
        for i in range(self.modal_num):
            FFN = MLP(
                ...
                parallel_config=parallel_config)
            router = Router(d_model=hidden_size, moe_config=moe_config, specific_expert_num=self.specific_expert_num, routing_policy=None,
                                 training=True, parallel_config=parallel_config)
            self.special_ffn.append(FFN)
            self.special_router.append(router)
        ...
        # self.split_3d = P.Split(axis=0,output_num=self.modal_num).shard(((1, 1, self.dp),))
        # self.cat_3d = P.Concat(axis=0).shard(((1, 1, self.dp), (1, 1, self.dp)))
        # self.add_3d = P.Add().shard(((1, 1, self.dp), (1, 1, self.dp)))
    ...
    def construct(self, modal_input):
        ...
        modal_list = self.split_3d(modal_input)
        special_out_list = []
        special_aux_loss = 0
        for i in range(len(modal_list)):
            special_out_i, aux_loss_i = self._construct(self.special_router[i],self.special_ffn[i],modal_list[i],self.specific_expert_num, self.transpose_2dim_ep, self.transpose_4dim_ep)
            special_out_list.append(special_out_i)
            special_aux_loss= self.add(special_aux_loss, aux_loss_i)
        if self.modal_num ==3:
            special_out= self.cat_3d((special_out_list[0],special_out_list[1]))
            special_out= self.cat_3d((special_out,special_out_list[2]))
        elif self.modal_num ==4:
            special_out= self.cat_3d((special_out_list[0],special_out_list[1]))
            special_out= self.cat_3d((special_out,special_out_list[2]))
            special_out= self.cat_3d((special_out,special_out_list[3]))

        output = self.add_3d(special_out,cross_out)
        output = self.add_3d(output,public_out)

        special_aux_loss = self.div(special_aux_loss,len(modal_list))
