...
from ringmoe_framework.loss.loss import MSELoss
...
from ringmoe_framework.models.layers.patch import Patchify, UnPatchify
...
class VisionTransformerForMae(Vit):
    ...
    def construct(self, img, unmask_index):
        ...
        aux_loss = None
        if self.use_moe:
            encoded_tokens, aux_loss = self.encoder(unmask_tokens, self.encoder_input_mask)
        else:
            encoded_tokens = self.encoder(unmask_tokens, self.encoder_input_mask)

        encoded_tokens = self.norm(encoded_tokens)
        return encoded_tokens, aux_loss
