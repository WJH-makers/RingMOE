...
# ============================================================================
"""build lr"""
from ringmoe_framework.lr.lr_schedule import CosineDecayLR, WarmUpLR, \
...
def build_lr(config):
    ...
    warmup_steps = int(lr_config.warmup_steps)
...
def _check_lr_config(config, device_num=1, batch_size=128, arch="simmim"):
    if arch in ('simmim', "ringmo_framework", "ringmo_mm", "simmim_single_moe", "simmim_moe"):
        config.base_lr = (config.base_lr * batch_size) / 512
        config.min_lr = (config.min_lr* batch_size) / 512
        config.warmup_lr = (config.warmup_lr * batch_size) / 512
    if arch == 'mae':
