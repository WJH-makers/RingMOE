...
def interpolate_pos_embed(model, checkpoint_model):
    if 'pos_embed' in checkpoint_model:
        ...
        if orig_size != new_size:
            ...
            # Use SciPy cubic interpolation (order=3) to resize [B, C, H, W].
            pos_np = pos_tokens.asnumpy()
            zoom_factors = (1.0, 1.0, float(new_size) / float(orig_size), float(new_size) / float(orig_size))
            pos_np = zoom(pos_np, zoom=zoom_factors, order=3, mode="nearest")
            pos_tokens = Tensor(pos_np, mstype.float32)

            pos_tokens = P.Transpose()(pos_tokens, (0, 2, 3, 1))
            pos_tokens = P.Reshape()(pos_tokens, (-1, new_size * new_size, embedding_size))
